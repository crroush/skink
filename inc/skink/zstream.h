...
struct zstream {
  ...
  zstream(size_t size = kDefaultSize) : buffer_(size) {
 ...
 private:
  ...
  struct MappedBuffer {
    ...
    MappedBuffer(size_t size) { map(size); }
    ...
    MappedBuffer(MappedBuffer &&b) : MappedBuffer() {
  ...
  struct AtomicInt64 {
    AtomicInt64(int64_t value = 0) : value_(value) {}

    AtomicInt64(AtomicInt64 &&b) { *this = static_cast<int64_t>(b); }

    int64_t value() const { return static_cast<int64_t>(*this); }

    operator int64_t() const { return value_.load(std::memory_order_acquire); }
  ...
  int64_t rdavail(int64_t offset) const { return wroffset_ - offset; }
