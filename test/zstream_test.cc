...
template <typename T>
ssize_t borrow_writer(zstream& stream, ssize_t nbyte, int seed, bool randomize) {
    constexpr int64_t kBufSamp = 32768;
    constexpr int64_t kBufByte = kBufSamp*sizeof(T);
...
template <typename T>
bool borrow_reader(zstream& stream, int id, ssize_t nbyte, int seed, bool randomize) {
    constexpr int64_t kBufSamp = 32768;
    constexpr int64_t kBufByte = kBufSamp*sizeof(T);
...
bool TestStream(int num_bytes, int num_reader, bool randomize) {
    ...
    for (ssize_t ii=0; ii < num_reader; ii++) {
        results.emplace_back(
            std::async(std::launch::async, reader<uint64_t>,
                std::ref(stream), stream.add_reader(), num_bytes, 1, randomize));
    }

    // Create writers.
    std::future<ssize_t> resultw = std::async(std::launch::async,
        writer<uint64_t>, std::ref(stream), num_bytes, 1, randomize);
...
bool TestBorrowStream(int num_bytes, int num_reader, bool randomize) {
    ...
    for (ssize_t ii=0; ii < num_reader; ii++) {
        results.emplace_back(
            std::async(std::launch::async, borrow_reader<uint64_t>,
                std::ref(stream), stream.add_reader(), num_bytes, 1, randomize));
    }

    // Create writers.
    std::future<ssize_t> resultw = std::async(std::launch::async,
        borrow_writer<uint64_t>, std::ref(stream), num_bytes, 1, randomize);
